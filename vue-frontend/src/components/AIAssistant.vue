...
const loadConfig = async () => {
  try {
    let config;
    try {
      if (window.ankiGetAIConfig) {
        config = window.ankiGetAIConfig();
      } else {
        // fallback to localStorage if Anki bridge not available
        config = {
          api_key: localStorage.getItem("ai-assistant-api-key") || "",
          model_name:
            localStorage.getItem("ai-assistant-model-name") ||
    ...
    needsApiKey.value = !config.api_key;
    if (config.api_key) apiKey.value = config.api_key;
    if (config.model_name) modelName.value = config.model_name;
  } catch (error) {
...
const saveSettings = () => {
  ...
  try {
    if (window.ankiSaveAIConfig) {
      window.ankiSaveAIConfig(apiKey.value.trim(), modelName.value);
    } else {
      try {
    ...
    if (
      cardContent.value?.trim?.() ||
      currentCardContent.value?.text?.trim?.()
    ) {
      ...
      for (const image of cardData.images) {
        try {
          ...
          const mimeType = image.src.startsWith("data:")
            ? image.src.match(/data:([^;]+)/)?.[1] || "image/jpeg"
...
const sendMessage = async () => {
  ...
  try {
    const cardData =
      isAutoAnalysis.value && currentCardContent.value?.hasImages
        ? currentCardContent.value
        : null;
    const aiResponse = await callGeminiAPI(
      buildContextualPrompt(currentPrompt, isAutoAnalysis.value),
      cardData
    );
    ...
    const errorMessage = createMessage(
      "error",
      "error: could not get ai response. please check your api key and try again."
    );
...
const triggerAnalysis = (isManual = false) => {
  ...
  const hasText =
    cardContent.value.trim() || currentCardContent.value?.text?.trim();
...
onMounted(() => {
  ...
  const observer = new MutationObserver(() => {
    ...
    const contentChanged =
      newCardContent?.text !== oldCardContent?.text ||
      newCardContent?.images?.length !== oldCardContent?.images?.length;
    ...
    if (newCardContent && contentChanged) {
      ...
      if (
        window.aiPanelVisible &&
        !needsApiKey.value &&
        (newCardContent.text || newCardContent.hasImages)
      ) {
        autoSendCardAnalysis();
      }
    }
...
<template>
  <div class="grok-chat">
    <div
      v-if="showSettings"
      class="modal-overlay"
      @click.self="!needsApiKey && (showSettings = false)"
    >
      <div class="modal">
    ...
    <div
      v-if="showExpandedControls"
      class="expanded-overlay"
      @click="closeExpandedControls"
    ></div>

    <div
      class="expandable-controls"
      :class="{ expanded: showExpandedControls }"
    >
    ...
    <div class="input-area">
      <div class="input-container">
        ...
        <button
          @click="sendMessage"
          class="send-btn"
          :disabled="!cardContent.trim() || isLoading"
        >
